...
// RolesDir is the relative directory within an SDK project where Puppet roles
// are stored.
...
if ! which puppet-galaxy > /dev/null; then
	echo "puppet needs to be installed"
	exit 1
fi

echo "Initializing role skeleton..."
puppet-galaxy init --init-path={{.Input.AbsProjectPath}}/{{.RolesDir}}/ {{.Resource.Kind}}
`
