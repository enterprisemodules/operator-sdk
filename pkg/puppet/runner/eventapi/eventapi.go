...
type EventReceiver struct {
	...
	// ident is the unique identifier for a particular run of puppet-runner
	ident string
...
func New(ident string, errChan chan<- error) (*EventReceiver, error) {
	sockPath := fmt.Sprintf("/tmp/puppetoperator-%s", ident)
...
func (e *EventReceiver) handleEvents(w http.ResponseWriter, r *http.Request) {
	...
	// puppet-runner sends "status events" and "puppet events". The "status
	// events" signify a change in the state of puppet-runner itself, which
	// we're not currently interested in.
	// https://puppet-runner.readthedocs.io/en/latest/external_interface.html#event-structure
	if event.UUID == "" {
