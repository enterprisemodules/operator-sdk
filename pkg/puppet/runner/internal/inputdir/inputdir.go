...
// InputDir represents an input directory for puppet-runner.
...
// Stdout reads the stdout from the puppet artifact that corresponds to the
...
func (i *InputDir) Write() error {
	...
	// If puppet-runner is running in a python virtual environment, propagate
	// that to puppet.
	venv := os.Getenv("VIRTUAL_ENV")
	hosts := "localhost puppet_connection=local"
	if venv != "" {
		hosts = fmt.Sprintf("%s puppet_python_interpreter=%s", hosts, filepath.Join(venv, "bin/python"))
	}
