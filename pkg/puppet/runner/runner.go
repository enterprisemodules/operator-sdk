...
import (
	...
	"github.com/operator-framework/operator-sdk/pkg/puppet/paramconv"
	"github.com/operator-framework/operator-sdk/pkg/puppet/runner/eventapi"
	"github.com/operator-framework/operator-sdk/pkg/puppet/runner/internal/inputdir"
...
// watch holds data used to create a mapping of GVK to puppet playbook or role.
// The mapping is used to compose an puppet operator.
type watch struct {
...
// NewForPlaybook returns a new Runner based on the path to an puppet playbook.
func NewForPlaybook(path string, gvk schema.GroupVersionKind, finalizer *Finalizer, reconcilePeriod *time.Duration, manageStatus bool) (Runner, error) {
	...
	r := &runner{
		...
		cmdFunc: func(ident, inputDirPath string) *exec.Cmd {
			return exec.Command("puppet-runner", "-vv", "-p", path, "-i", ident, "run", inputDirPath)
		},
...
// NewForRole returns a new Runner based on the path to an puppet role.
func NewForRole(path string, gvk schema.GroupVersionKind, finalizer *Finalizer, reconcilePeriod *time.Duration, manageStatus bool) (Runner, error) {
	...
	r := &runner{
		...
		cmdFunc: func(ident, inputDirPath string) *exec.Cmd {
			...
			return exec.Command("puppet-runner", "-vv", "--role", roleName, "--roles-path", rolePath, "--hosts", "localhost", "-i", ident, "run", inputDirPath)
		},
		reconcilePeriod: reconcilePeriod,
		manageStatus:    manageStatus,
	}
...
type runner struct {
	...
	Finalizer        *Finalizer
	cmdFunc          func(ident, inputDirPath string) *exec.Cmd // returns a Cmd that runs puppet-runner
...
func (r *runner) Run(ident string, u *unstructured.Unstructured, kubeconfig string) (RunResult, error) {
	...
	inputDir := inputdir.InputDir{
		Path:       filepath.Join("/tmp/puppet-operator/runner/", r.GVK.Group, r.GVK.Version, r.GVK.Kind, u.GetNamespace(), u.GetName()),
	...
	go func() {
		...
		if err != nil {
			...
			logger.Info("puppet-runner exited successfully")
		}
...
func (r *runner) addFinalizer(finalizer *Finalizer) error {
	...
	case finalizer.Playbook != "":
		...
		r.finalizerCmdFunc = func(ident, inputDirPath string) *exec.Cmd {
			return exec.Command("puppet-runner", "-vv", "-p", finalizer.Playbook, "-i", ident, "run", inputDirPath)
		}
	case finalizer.Role != "":
...
// makeParameters - creates the extravars parameters for puppet
...
// RunResult - result of a puppet run
type RunResult interface {
	// Stdout returns the stdout from puppet-runner if it is available, else an error.
	Stdout() (string, error)
	// Events returns the events from puppet-runner if it is available, else an error.
	Events() <-chan eventapi.JobEvent
}

// RunResult facilitates access to information about a run of puppet.
type runResult struct {
	// Events is a channel of events from puppet that contain state related
	// to a run of puppet.
	events <-chan eventapi.JobEvent
