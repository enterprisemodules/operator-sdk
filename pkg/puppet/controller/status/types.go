...
// PuppetResult - encapsulation of the puppet result.
type PuppetResult struct {
	Ok               int                `json:"ok"`
...
// NewPuppetResultFromStatusJobEvent - creates a Puppet status from job event.
func NewPuppetResultFromStatusJobEvent(je eventapi.StatusJobEvent) *PuppetResult {
	// ok events.
	a := &PuppetResult{TimeOfCompletion: je.Created}
...
// NewPuppetResultFromMap - creates a Puppet status from a job event.
func NewPuppetResultFromMap(sm map[string]interface{}) *PuppetResult {
...
// Condition - the condition for the puppet operator.
type Condition struct {
	Type               ConditionType      `json:"type"`
	Status             v1.ConditionStatus `json:"status"`
	LastTransitionTime metav1.Time        `json:"lastTransitionTime"`
	PuppetResult       *PuppetResult      `json:"puppetResult,omitempty"`
...
func createConditionFromMap(cm map[string]interface{}) Condition {
	...
	asm, ok := cm["puppetResult"].(map[string]interface{})
	var puppetResult *PuppetResult
	if ok {
		puppetResult = NewPuppetResultFromMap(asm)
	}
