...
import (
	...
	"github.com/operator-framework/operator-sdk/pkg/puppet/controller"
	...
	"github.com/operator-framework/operator-sdk/pkg/puppet/runner/fake"
...
func TestReconcile(t *testing.T) {
					...
					"status": map[string]interface{}{
						"conditions": []interface{}{
							map[string]interface{}{
								"status": "True",
								"type":   "Running",
								"puppetResult": map[string]interface{}{
									"changed":    int64(0),
									"failures":   int64(0),
									"ok":         int64(0),
									"skipped":    int64(0),
	...
	for _, tc := range testCases {
		t.Run(tc.Name, func(t *testing.T) {
			var aor reconcile.Reconciler = &controller.PuppetOperatorReconciler{
				GVK:             tc.GVK,
				Runner:          tc.Runner,
				Client:          tc.Client,
			...
			if tc.ExpectedObject != nil {
				...
				expectedStatus := puppetstatus.CreateFromMap(sMap)
				sMap, _ = actualObject.Object["status"].(map[string]interface{})
				actualStatus := puppetstatus.CreateFromMap(sMap)
				...
				for _, c := range expectedStatus.Conditions {
					actualCond := puppetstatus.GetCondition(actualStatus, c.Type)
					...
					if c.PuppetResult == nil && actualCond.PuppetResult != nil {
						t.Fatalf("puppet result did not match expected: %v\nactual: %v", c.PuppetResult, actualCond.PuppetResult)
					}
					if c.PuppetResult != nil {
						if !reflect.DeepEqual(c.PuppetResult, actualCond.PuppetResult) {
							t.Fatalf("puppet result did not match expected: %v\nactual: %v", c.PuppetResult, actualCond.PuppetResult)
						}
					}
				}
